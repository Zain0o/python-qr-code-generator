...
from matrix_layout import (
    ...
    QRMatrix as QRMatrixWithPlaceholders,
    FinalQRMatrix
)
...
def assemble_qr_matrix(text: str, explain=False):
    ...
    final_bits = ''.join(data_cw + ecc_cw)
    if version == 2:
        final_bits += '0' * 7
    ...
    int_matrix = [[v if isinstance(v, int) else 0 for v in row] for row in masked_matrix]
    ...
    if request.method == "POST":
        if not text_val:
            ...
            try:
                ...
                qr_html = matrix_to_html(qr_data["final"], fg=fg_color, bg=bg_color, shape=shape, size=size, frame=frame, filter_mode=filter_mode)
                if explain_steps:
                    step_images = [
